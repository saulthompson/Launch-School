...
def calculate(num_1, num_2, operator)
	case operator
		when '/'
			return division_calculator(num_1, num_2)
		when '*'
			return format_decimal_places!(num_1.to_f() * num_2.to_f())
		when '+'
			return format_decimal_places!(num_1.to_f() + num_2.to_f())
		when "-"
			return format_decimal_places!(num_1.to_f() - num_2.to_f())
...
loop do
	...
	display(num_1 + " " + operator + " ...")
	num_2 = get_num()
	
	calculation = calculate(num_1, num_2, operator)
	...
	continue?() ? (system('clear'); next) :
	(system('clear'); prompt("goodbye", name: user_name))
	break
end
